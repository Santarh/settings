...
if has( "autocmd" )
	...
	function! UpdateBackupFile()
		let dir = strftime("~/.vimbackup"."/%Y/%m/%d", localtime())
		if !isdirectory(dir)
			let retval = system("mkdir -p ".dir)
			let retval = system("chown sumimau:staff ".dir)
		endif
...
"	au BufWritePost *.rb :%s /\t/    /g
"	au BufWritePost *.erb :%s /\t/    /g
"	au BufWritePost *.html :%s /\t/    /g
"	au BufWritePost *.js :%s /\t/    /g
endif

"autocmd BufNewFile,BufRead *.shader set filetype=cg
"autocmd BufNewFile,BufRead *.cginc set filetype=cg
"autocmd BufNewFile,BufRead *.vsh set filetype=cg
"autocmd BufNewFile,BufRead *.fsh set filetype=cg
set rtp+=~/.vim/syntax/cg.vim
set rtp+=~/.vim/syntax/fx.vim
set rtp+=~/.vim/syntax/jsx.vim
